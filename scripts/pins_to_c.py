# pins_to_c.py 17.8.2020/pekka
# Converts hardware IO specification(s) written in JSON to C source and header files.
import json
import os
import sys

pin_types = {
    "inputs" : "PIN_INPUT",
    "outputs" : "PIN_OUTPUT",
    "analog_inputs" : "PIN_ANALOG_INPUT",
    "analog_outputs" : "PIN_ANALOG_OUTPUT",
    "pwm" : "PIN_PWM",
    "spi" : "PIN_SPI",
    "i2c" : "PIN_I2C",
    "timers" : "PIN_TIMER",
    "cameras" : "PIN_CAMERA",
    "uart" : "PIN_UART"}

prm_type_list = {
    "pull-up": "PIN_PULL_UP",
    "pull-down": "PIN_PULL_DOWN",
    "touch": "PIN_TOUCH",
    "frequency": "PIN_FREQENCY",
    "frequency-kHz": "PIN_FREQENCY_KHZ",
    "frequency-MHz": "PIN_FREQENCY_MHZ",
    "resolution": "PIN_RESOLUTION",
    "init": "PIN_INIT",
    "hpoint": "PIN_HPOINT",
    "interrupt": "PIN_INTERRUPT_ENABLED",
    "timer": "PIN_TIMER_SELECT",
    "tgroup": "PIN_TIMER_GROUP_SELECT",
    "miso": "PIN_MISO",
    "mosi": "PIN_MOSI",
    "sclk": "PIN_SCLK",
    "sda": "PIN_SDA",
    "scl": "PIN_SCL",
    "cs": "PIN_CS",
    "dc": "PIN_DC",
    "rx": "PIN_RX",
    "tx": "PIN_TX",
    "tc": "PIN_TRANSMITTER_CTRL",
    "speed": "PIN_SPEED",
    "speed-kbps": "PIN_SPEED_KBPS",
    "flags": "PIN_FLAGS",
    "pin-a": "PIN_A",
    "pin-b": "PIN_B",
    "pin-c": "PIN_C",
    "pin-d": "PIN_D",
    "pin-e": "PIN_E",
    "bank-a": "PIN_A_BANK",
    "bank-b": "PIN_B_BANK",
    "bank-c": "PIN_C_BANK",
    "bank-d": "PIN_D_BANK",
    "bank-e": "PIN_E_BANK",
    "min": "PIN_MIN",
    "max": "PIN_MAX",
    "smin": "PIN_SMIN",
    "smax": "PIN_SMAX",
    "digs": "PIN_DIGS"}

def start_c_files():
    global ccontent, hcontent, hfilepath
    
    ccontent  = '/* This file is generated by pins_to_c.py script, do not modify. */\n'
    ccontent += '#include "pins.h"\n'
    hcontent  = '/* This file is generated by pins_to_c.py script, do not modify. */\n'
    path, fname = os.path.split(hfilepath)
    fname, ext = os.path.splitext(fname)
    macroname = 'IOC_' + fname.upper() + '_INCLUDED'
    hcontent += '#ifndef ' + macroname + '\n'
    hcontent += '#define ' + macroname + '\n'
    hcontent += 'OSAL_C_HEADER_BEGINS\n\n'

def finish_c_files():
    global hcontent
    
    hcontent += '\nOSAL_C_HEADER_ENDS\n'
    hcontent += '#endif\n'

def write_pin_to_c_header(pin_name):
    global hcontent
    hcontent += "    Pin " + pin_name + ";\n"

def write_pin_to_c_source(pin_type, pin_name, pin_attr):
    global hcontent, ccontent, savedcontent
    global known_groups, prefix, ccontent, c_prm_comment_written
    global nro_pins, pin_nr, define_list, device_list, driver_list, bus_list, bus_pin_list

    # Generate C parameter list for the pin
    c_prm_list = "{PIN_RV, PIN_RV}, {PIN_RV, PIN_RV}"
    c_prm_list_has_interrupt = False
    c_prm_list_has_scaling = False
    for attr, value in pin_attr.items():
        c_attr_name = prm_type_list.get(attr, "")
        if c_attr_name != "":
            c_prm_list += ", {" + c_attr_name + ", "
            if c_attr_name == 'PIN_SPEED' or c_attr_name == 'PIN_SPEED_KBPS':
                c_prm_list += str(int(value)//100) + '}'
            else:
                c_prm_list += str(value) + '}'

            if c_attr_name == 'PIN_DIGS' or c_attr_name == 'PIN_SMAX' or c_attr_name == 'PIN_SMIN':
                c_prm_list_has_scaling = True

            if c_attr_name == 'PIN_INTERRUPT_ENABLED':
                c_prm_list_has_interrupt = True

        elif attr != 'name' and attr != 'addr' and attr != 'bank' and attr != 'group' and attr != 'device' and attr != 'driver':
            print("pins_to_c.py: Pin '" + pin_name + "' has unknown attribute '" + attr + "', ignored.")

    if c_prm_list_has_interrupt == False and pin_type == 'timers':
        c_prm_list_has_interrupt = True
        c_prm_list += ", {PIN_INTERRUPT_ENABLED, 1}"

    # If we have C parameters, write to C file
    c_prm_array_name = "OS_NULL"
    if c_prm_comment_written == False:
        ccontent += "\n/* Parameters for " + pin_type + " */\n"
        c_prm_comment_written = True
    c_prm_array_name = prefix + "_" + pin_type + "_" + pin_name + "_prm"
    ccontent += "static PinPrmValue " + c_prm_array_name + "[]"
    ccontent += "= {" + c_prm_list + "};\n"

    define_text = prefix + '_' + pin_type + '_' + pin_name
    define_list.append(define_text.upper() + ' "' + pin_name + '"')

    full_pin_name = prefix + '.' + pin_type + '.' + pin_name

    # Write pin type
    savedcontent += '    {' + pin_types[pin_type] + ", "

    # Write pin address, merge addr and bank
    bank = pin_attr.get("bank", "0")
    savedcontent += str(bank) + ", "
    addr = pin_attr.get("addr", "0")
    savedcontent += str(addr) + ", "

    # Write pointer to parameter array, if any
    savedcontent += c_prm_array_name + ", "
    if c_prm_array_name == "OS_NULL":
        savedcontent += "0, "
    else:
        savedcontent += "sizeof(" + c_prm_array_name + ")/sizeof(PinPrmValue), "

    # Write flags, like PIN_SCALING_SET
    if c_prm_list_has_scaling:
        savedcontent += "PIN_SCALING_SET, "
    else:
        savedcontent += "0, "

    # If IO pin belongs to group, setup linked list
    group = pin_attr.get("group", None)
    if group is None:
        savedcontent += "OS_NULL"
    else:
        g = known_groups.get(group, None)
        if g is None:
            g = "OS_NULL"
            known_groups.update( {group : full_pin_name} )
            define_text = prefix + '_' + group + '_GROUP'
            define_list.append(define_text.upper() + ' "' + group + '"')

        else:
            known_groups[group] = full_pin_name
            g = "&" + g

        savedcontent += g

    if pin_name in signallist:
        savedcontent += ', ' + signallist[pin_name]
    else:
        savedcontent += ', OS_NULL'

    # If IO pin is on SPI or I2C device
    bus_device = pin_attr.get("device", None)
    if bus_device != None:
        savedcontent += ' PINS_DEVCONF_PTR('
        savedcontent += 'pins_device_' + bus_device.replace('.',  '_')
        savedcontent += ')'
        tmp = bus_device.split('.')
        bus_pin_list.append( (tmp[1], full_pin_name) )

    else:
        savedcontent += ' PINS_DEVCONF_NULL'

    # If IO pin is a SPI or I2C device
    driver = pin_attr.get("driver", None)
    if driver != None:
        if pin_type == 'i2c':
            bus_id = pin_type + '_' + str(pin_attr.get('bank', 0))
        else:
            bus_id = pin_type + '_' + str(pin_attr.get('sclk', 0))

        next_device = bus_list.get(bus_id, 'OS_NULL')
        bus_list[bus_id] = '&' + 'pins_device_' + pin_type + '_' + pin_name
        device_list[pin_name] = (driver, pin_type, pin_name, next_device, bus_id)
        driver_list[driver] = 'x'

    if c_prm_list_has_interrupt:
        intconf_struct_name = "pin_" + pin_name + "_intconf"
        ccontent += "PINS_INTCONF_STRUCT("
        ccontent += intconf_struct_name
        ccontent += ")\n"
        savedcontent += ' PINS_INTCONF_PTR('
        savedcontent += intconf_struct_name
        savedcontent += ')'
    else:
        savedcontent += ' PINS_INTCONF_NULL'

    savedcontent += "}"
    if pin_nr <= nro_pins:
        savedcontent += ","

    savedcontent += ' /* ' + pin_name + ' */\n'

def write_device_list(device_list, driver_list, bus_list):
    global hcontent, ccontent

    ccontent += '\n#if PINS_SPI || PINS_I2C\n'
    ccontent += '\n/* SPI and I2C bus structures */\n'
    next_bus = 'OS_NULL'
    for bus_name, data in bus_list.items():
        ccontent += 'PinsBus pins_bus_' + bus_name + ' = {'
        if bus_name[:3] == 'spi':
            ccontent += 'PINS_SPI_BUS, '
        else:
            ccontent += 'PINS_I2C_BUS, '
        ccontent += data + ', ' + next_bus + '};\n'
        next_bus = '&pins_bus_' + bus_name

    ccontent += '\n/* Device bus main structure */\n'
    ccontent += 'PinsDeviceBus pins_devicebus = {' + next_bus + '};\n'

    ccontent += '\n/* SPI and I2C device structures */\n'
    for device_name, data in device_list.items():
        ccontent += 'PinsBusDevice pins_device_' + data[1] + '_' + data[2] + ' = {'
        ccontent += '&' + prefix + '.' + data[1] + '.' + data[2] + ', '
        ccontent += '&pins_bus_' +  data[4] + ', '
        ccontent += data[3] + ', '
        ccontent += '&' + data[0] + '_initialize_pin, '
        ccontent += '&' + data[0] + '_gen_req, '
        ccontent += '&' + data[0] + '_proc_resp, '
        ccontent += '&' + data[0] + '_set, '
        ccontent += '&' + data[0] + '_get};\n'

    ccontent += '\n/* Initialize all SPI and I2C bus devices */\n'
    ccontent += 'void pins_initialize_bus_devices(void)\n{\n'
    for bus_name, data in bus_list.items():
        ccontent += '    pins_init_bus(&pins_bus_' + bus_name + ');\n'
    for driver_name, data in driver_list.items():
        ccontent += '    ' + driver_name + '_initialize_driver();\n'
    for device_name, data in device_list.items():
        ccontent += '    ' + data[0] + '_initialize_device(&pins_device_' + data[1] + '_' + data[2] + ');\n'
    for pin in bus_pin_list:
        data = device_list[pin[0]];
        ccontent += '    ' + data[0] + '_initialize_pin(&' + pin[1] + ');\n'
    ccontent += '}\n'

    ccontent += '#endif\n'

    hcontent += '\n/* SPI and I2C initialization */\n'
    hcontent += '#if PINS_SPI || PINS_I2C\n'

    # Moved to pins_devicebus.h
    # hcontent += '\n/* Device bus main structure */\n');
    # hcontent += 'extern PinsDeviceBus pins_devicebus;\n')

    hcontent += '\n/* SPI and I2C bus structures */\n'
    for bus_name, data in bus_list.items():
        hcontent += 'extern PinsBus pins_bus_' + bus_name + ';\n'

    hcontent += '\n/* SPI and I2C device structures */\n'
    for device_name, data in device_list.items():
        hcontent += 'extern PinsBusDevice pins_device_' + data[1] + '_' + data[2] + ';\n'

    for device_name, data in device_list.items():
        hcontent += '\n/* ' + data[0] + ' driver functions  */\n'
        hcontent += 'void ' + data[0] + '_initialize_driver(void);\n'
        hcontent += 'void ' + data[0] + '_initialize_device(struct PinsBusDevice *device);\n'
        hcontent += 'void ' + data[0] + '_initialize_pin(const struct Pin *pin);\n'
        hcontent += 'osalStatus ' + data[0] + '_gen_req(struct PinsBusDevice *device);\n'
        hcontent += 'osalStatus ' + data[0] + '_proc_resp(struct PinsBusDevice *device);\n'
        hcontent += 'osalStatus ' + data[0] + '_set(struct PinsBusDevice *device, os_short addr, os_int value);\n'
        hcontent += 'os_int ' + data[0] + '_get(struct PinsBusDevice *device, os_short addr, os_char *state_bits);\n'

    # for driver_name, func_decl in driver_list.items():
    #     hcontent += func_decl)
    hcontent += '#endif\n'

def write_linked_list_heads():
    global prefix, known_groups
    global hcontent, ccontent

    isfirst = True
    for g, value in known_groups.items():
        varname = prefix + "_" +  g + "_group"
        if isfirst:
            ccontent += "\n/* Application's pin groups (linked list heads) */\n"
            hcontent += "\n/* Application's pin groups (linked list heads) */\n"
            isfirst = False
        ccontent += "OS_CONST Pin *" + varname + " = &" + value + ";\n"
        hcontent += "extern OS_CONST_H Pin *" + varname + ";\n"

def process_pin(pin_type, pin_attr):
    global device_name, ccontent, savedcontent
    global pin_nr

    pin_name = pin_attr.get("name", None)
    if pin_name == None:
        print("pins_to_c.py: 'name' not found for pin in " + device_name + " " + pin_type)
        exit()

    if pin_nr == 1:
        savedcontent += ', &' + prefix + '.' + pin_type + '.' + pin_name + '}, /* ' + pin_type + ' */\n'
    pin_nr = pin_nr + 1

    write_pin_to_c_header(pin_name)
    write_pin_to_c_source(pin_type, pin_name, pin_attr)

def count_pins(pins):
    count = 0
    for pin in pins:
        count = count + 1
    return count

def process_group_block(group):
    global hcontent, ccontent
    global nro_groups, group_nr, ccontent, savedcontent, c_prm_comment_written
    global nro_pins, pin_nr, pin_group_list

    pin_type = group.get("name", None)
    if pin_type == None:
        print("pins_to_c.py: 'name' not found for group in " + device_name)
        exit()

    if pin_types.get(pin_type, None) == None:
        print("pins_to_c.py: Pin group '"+ pin_type + "' ignored.")
        return;

    hcontent += '\n  struct\n  {\n'
    hcontent += '    PinGroupHdr hdr;\n'

    pins = group.get("pins", None)
    if pins == None:
        print("pins_to_c.py: 'pins' not found for " + device_name + " " + pin_type)
        exit()

    group_nr = group_nr + 1

    pin_group_list.append(prefix + '.' + pin_type)

    nro_pins = count_pins(pins)
    pin_nr = 1
    c_prm_comment_written = False

    savedcontent += '\n  {{' + str(nro_pins)

    for pin in pins:
        process_pin(pin_type, pin)

    savedcontent += '  }'
    if group_nr <= nro_groups:
        savedcontent += ','
    savedcontent += '\n'

    hcontent += '  }\n  ' + pin_type + ';\n'

def count_groups(groups):
    count = 0
    for group in groups:
        count = count + 1
    return count

def list_signals_in_mblk(mblk, device_name):
    global signallist
    groups = mblk.get("groups", None)
    mblk_name = mblk.get("name", None);

    for group in groups:
        signals  = group.get("signals", None);
        if signals != None:
            for signal in signals:
                signal_name = signal.get('name', None)
                if signal_name is not None:
                    signallist.update({signal_name : '&' + device_name + '.' + mblk_name + '.' + signal_name})

def list_signals_in_file(path):
    signals_file = open(path, "r")
    if signals_file:
        data = json.load(signals_file)
        device_name = data.get("name", None)
        if device_name == None:
            print("pins_to_c.py: device 'name' not found in " + path)
            return
        mblks = data.get("mblk", None)
        if mblks == None:
            print("pins_to_c.py: 'mblk' not found in "  + path)
            return

        for mblk in mblks:
            list_signals_in_mblk(mblk, device_name)

    else:
        printf ("pins_to_c.py: Opening file " + path + " failed")

def process_io_device(io):
    global hcontent, ccontent, savedcontent
    global device_name, known_groups, prefix, signallist, device_list, driver_list, bus_list, bus_pin_list
    global nro_groups, group_nr, ccontent, pin_group_list, define_list

    device_name = io.get("name", "ioblock")
    groups = io.get("groups", None)
    prefix = io.get("prefix", "pins")
    pin_group_list = []
    define_list = []

    signallist = {}
    if signalspath != None:
        list_signals_in_file(signalspath)

    device_list = {}
    driver_list = {}
    bus_list = {}
    bus_pin_list = []

    hcontent += "/* " + device_name.upper() + " IO configuration structure */\n"
    hcontent += 'typedef struct\n{'

    if groups == None:
        print("pins_to_c.py: 'groups' not found for " + device_name)
        exit()

    nro_groups = count_groups(groups)
    group_nr = 1;

    savedcontent = "\n/* " + device_name.upper() + " IO configuration structure */\n"
    savedcontent += 'OS_CONST ' + prefix + '_t ' + prefix + ' =\n{'

    known_groups = {}

    for group in groups:
        process_group_block(group)

    savedcontent += '};\n\n'
    ccontent += savedcontent

    list_name = prefix + "_group_list"
    ccontent += '/* List of pin type groups */\n'
    ccontent += 'static OS_CONST PinGroupHdr * OS_CONST ' + list_name + '[] =\n{\n  '
    isfirst = True
    for p in pin_group_list:
        if not isfirst:
            ccontent += ',\n  '
        isfirst = False
        ccontent += '&' + p + '.hdr'
    ccontent += '\n};\n\n'

    ccontent += '/* ' + device_name.upper() + ' IO configuration top header structure */\n'
    ccontent += 'OS_CONST IoPinsHdr pins_hdr = {' + list_name + ', sizeof(' + list_name + ')/' + 'sizeof(PinGroupHdr*)};\n'

    hcontent += '}\n' + prefix + '_t;\n\n'

    hcontent += "/* " + device_name.upper() + " IO configuration top header structure */\n"
    hcontent += 'extern OS_CONST_H IoPinsHdr ' + prefix + '_' + 'hdr;\n\n'

    hcontent += "/* Global " + device_name.upper() + " IO configuration structure */\n"
    hcontent += 'extern OS_CONST_H ' + prefix + '_t ' + prefix + ';\n'

    write_linked_list_heads()

    hcontent += "\n/* Name defines for pins and application pin groups (use ifdef to check if HW has pin) */\n"
    for d in define_list:
        hcontent += '#define ' +d + '\n'

    write_device_list(device_list, driver_list, bus_list)

def process_source_file(path):
    read_file = open(path, "r")
    if read_file:
        data = json.load(read_file)
        ioroot = data.get("io", None)
        if ioroot == None:
            print("pins_to_c.py: 'io' not found")
            exit()

        for io in ioroot:
            process_io_device(io)

    else:
        printf ("pins_to_c.py: Opening file " + path + " failed")

# Write a file only if content has changed
def write_file_only_if_changed(filepath, content):
    if (os.path.isfile(filepath)):
        hfile = open(filepath, "r")
        old_content = hfile.read()
        hfile.close()
        if (content == old_content):
            return

    print("Writing pins_to_c file " + filepath)

    hfile = open(filepath, "w")
    hfile.write(content)
    hfile.close()
    return

def mymain():
    global hcontent, ccontent
    global cfilepath, hfilepath, signalspath

    # Get options
    n = len(sys.argv)
    sourcefiles = []
    outpath = None
    signalspath = None
    expectpath = True
    for i in range(1, n):
        if sys.argv[i][0] == "-":
            if sys.argv[i][1] == "o":
                outpath = sys.argv[i+1]
                expectpath = False

            if sys.argv[i][1] == "s":
                signalspath = sys.argv[i+1]
                expectpath = False

        else:
            if expectpath:
                sourcefiles.append(sys.argv[i])

            expectpath = True

    if len(sourcefiles) < 1:
        print("pins_to_c.py: No source files")
        exit()

#    sourcefiles.append('/coderoot/iocom/examples/buster/config/pins/grumpy/pins_io.json')
#    outpath = '/coderoot/iocom/examples/buster/config/include/grumpy/pins_io.c'

#    sourcefiles.append('/coderoot/dehec-ref/dref/config/pins/yogurt/pins_io.json')
#    outpath = '/coderoot/dehec-ref/dref/config/include/yogurt/pins_io.c'

#     sourcefiles.append('/coderoot/pins/examples/jane/config/pins/carol/pins_io.json')
#     outpath = '/coderoot/pins/examples/jane/config/include/carol/pins_io.c'

#    sourcefiles.append('/coderoot/iocom/examples/gina/config/pins/carol/gina-io.json')
#    outpath = '/coderoot/iocom/examples/gina/config/include/carol/gina-io.c'
#    signalspath = '/coderoot/iocom/examples/gina/config/signals/gina-signals.json'

    if outpath is None:
        outpath = sourcefiles[0]

    filename, file_extension = os.path.splitext(outpath)
    cfilepath = filename + '.c'
    hfilepath = filename + '.h'

    start_c_files()

    for path in sourcefiles:
        process_source_file(path)

    finish_c_files()

    write_file_only_if_changed(cfilepath, ccontent)
    write_file_only_if_changed(hfilepath, hcontent)

mymain()
